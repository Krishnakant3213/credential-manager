    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <form method="post" action="{{ route('users.update', $user->getKey()) }}" class="mt-6 space-y-6">
                    @csrf
                    @method('PATCH')
                                ...
                                <table class="min-w-full divide-y divide-gray-300" id="dynamicTable">
                                    ...
                                    @if($user->credentials->isNotEmpty())
                                        @foreach($user->credentials as $credential)
                                            <tr>
                                                <input type="hidden" name="id" value="{{$credential->getKey()}}">
                                                ...
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                                    <x-text-input id="password" name="credential[0][password]"
                                                                  type="text"
